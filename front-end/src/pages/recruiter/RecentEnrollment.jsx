                        ...
                        <>
                            {!showStudySelection ? (
                                <div style={{ display: 'flex', gap: '0.75rem' }}>
                                    ...
                                    <button
                                        onClick={async () => {
                                            if (!confirm('Move this volunteer back to pre-screening?')) return;
                                            try {
                                                await api.post(`/enrollment/move-back/${volunteer.volunteer_id}`);
                                                onClose();
                                                window.location.reload();
                                            } catch (err) {
                                                alert(err.response?.data?.detail || 'Failed to move volunteer back');
                                            }
                                        }}
                                        style={{
                                            padding: '1rem',
                            ...
                            <div>
                                ...
                                {data.screening.filter(v =>
                                    ...
                                    <div className="glass-card" style={{ padding: '3rem 2rem', textAlign: 'center', background: 'linear-gradient(135deg, rgba(255,255,255,0.9) 0%, rgba(255,255,255,0.7) 100%)' }}>
                                        <Users size={56} color="#cbd5e1" style={{ marginBottom: '1rem' }} />
                                        <p style={{ color: '#64748b', fontSize: '1rem', fontWeight: '600' }}>
                                            {screeningSearch ? 'No matching volunteers found' : 'No pre-screening volunteers'}
                                        </p>
                                        {screeningSearch && (
                                            <button
                                                onClick={() => setScreeningSearch('')}
